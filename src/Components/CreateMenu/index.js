...
const CreateMenu = (props) => {
  ...
  const restaurantData = useSelector((state) => state.createRestaurant.restaurantData);
  const selectedCategoryIIID = useSelector((state) => state.selectedCategory);
  const localData = useSelector((state) => state.localData);

  const fetchetProducts = useSelector((state) => state.fetchProducts.fetchProducts);

  console.log('Local Data data from Create Menuu owner----MMMMM', restaurantData);
  console.log('Local Data data from Create Menuu owner----xxxxxxxxxxxxxxxxxx', fetchetProducts);
  const foodsMenu = Object.entries(restaurantData.menu.food).map(([id, category]) => ({
    id,
    ...category,
  }));
  const drinksMenu = Object.entries(restaurantData.menu.drinks).map(([id, category]) => ({
    id,
    ...category,
  }));
  ...
  const [unsubscribeProductsListener, setUnsubscribeProductsListener] = useState(null);
  const toggleHandle = () => {
  ...
  const handleAddCategory = async () => {
    ...
    if (value) {
      try {
        ...
        console.error('Fehler beim Hinzufügen der Kategorie zu Firestore:', error);
      }
    }
  };
