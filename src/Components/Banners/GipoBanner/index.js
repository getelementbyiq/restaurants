...
const GipoBanner = (props) => {
  ...
  // useEffect(() => {
  //   const tl = gsap.timeline({ repeat: -1 }); // Wiederholen der Animation unendlich oft

  //   // Texte auswählen und einblenden
  //   const texts = document.querySelectorAll(".bounce-text");
  //   texts.forEach((text, index) => {
  //     tl.to(text, {
  //       opacity: 1, // Text sichtbar machen
  //       duration: 0.8, // Dauer der Animation
  //       delay: index * 1.5, // Verzögerung für den nächsten Text
  //     })
  //       .to(text, {
  //         x: "+=100%", // Bewegung um 100% nach rechts
  //         duration: 0.8, // Dauer der Bewegung
  //         ease: "bounce.out", // Bounce-Effekt
  //       })
  //       .to(text, {
  //         opacity: 0, // Text ausblenden
  //         duration: 0.8, // Dauer der Animation
  //       }); // Offset für das Ausblenden
  //   });

  //   return () => {
  //     tl.kill(); // Animation stoppen, wenn die Komponente unmontiert wird
  //   };
  // }, []);
  ...
  return (
    <Grid
      sx={{
        display: "flex",
        height: scrolled ? "30vh" : "85vh",
        opacity: scrolled ? 0 : 1,
        transition: "height 250ms ease, opacity 300ms ease",
        alignItems: "center",
